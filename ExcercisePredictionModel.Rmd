...
The training dataset is split for later model verification.
...
The model is creating using an rpart from the caret package decision tree.

```{r model}
set.seed(123)
modelPML <- train(classe ~ ., method = "rpart", data = pml_train_cln_train)
```
...
modelPML_PCA <- train(classe ~ ., method = "rpart", data = pml_train_cln_PCA)
```
...
The decision tree model has an overall accuracy of `r cm$overall["Accuracy"]`. 
While the model using PCA has an overall accuracy of 
`r cm_pca$overall["Accuracy"]`. Using PCA doesn't show any advantage.
